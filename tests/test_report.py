...
def test_run_page_marks_what_changed_in_the_build(parsed, assessed):
    vs = views(parsed, assessed, ["icon_success", "icon_hang"])
    build = dict(vs[1].log.keyvalues["build"])
...
def test_index_marks_a_run_whose_build_changed(parsed, assessed):
    vs = views(parsed, assessed, ["icon_success", "icon_hang"])
    assert "exe-chg" not in report.render_index(vs, ["/tmp"], None, "Test").split("</style>")[1]
    build = {**vs[1].log.keyvalues["build"], "executable": "/scratch/other/bin/icon"}
    vs[1].log = replace(vs[1].log, keyvalues={**vs[1].log.keyvalues, "build": build})
    html = report.render_index(vs, ["/tmp"], None, "Test")
    assert '<a class="exe-chg" href="icon_hang.html#build"' in html
    assert 'title="Differs from the previous run in: executable"' in html
    assert html.count('class="exe-chg"') == 1
