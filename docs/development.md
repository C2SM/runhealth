...
(empty, truncated, and not a log file at all). No real log is committed, since
...
| `tables.py` | the two table shapes printed by models and MPI stacks |
| `health.py` | conversion of a `RunLog` into checks and a grade |
...
Open `docs/_build/html/index.html`. Add `-W` to turn warnings into errors,
which is what the [GitHub Actions workflow](https://github.com/C2SM/runhealth/blob/main/.github/workflows/docs.yml)
